...
- **野怪**：猩红石像 150 血（5 枪），蔚蓝石像 100 血（3 枪）。每局随机生成 6~8 只。
- **AI**：野怪有警戒区，进入范围后会追逐玩家。
- **后坐力**：连续射击时相机上抬 + 随机水平偏移，武器枪口上翘、准星扩散，松手自动回正。
- **结算**：全部击杀后结算总得分、击杀数、爆头数、命中率、最高连击，并给出评级。
...
- **血条是 3D Sprite + CanvasTexture**，随距离自动缩放，只在血量变化时重绘，不进每帧循环。
