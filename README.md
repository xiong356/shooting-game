...
- **AI**：野怪警戒区 24m（基本覆盖全场，进图即追），且红蓝分工明确：
  - **红怪**追到 2.6m 内抬臂前摇 0.35 秒（反应窗口），随后前冲挥击，命中 **25 伤**，冷却 1 秒；
  - **蓝怪**停在 12m 射程处站桩施法——后仰蓄力 + 双臂抬起 + 头顶宝石渐亮 0.4 秒预警，
    然后前倾掷出直线飞行的魔法弹，**10 伤**，冷却 1.5 秒。弹速 9 m/s（约 1.3 秒飞完 12 米），
    走位即可躲开，撞掩体会碎成蓝色粒子。
- **后坐力**：连续射击时相机上抬 + 随机水平偏移，武器枪口上翘、准星扩散，松手自动回正。
- **结算**：全歼野怪弹出胜利结算（评级 S）；血量归零则显示「你已阵亡」（评级 F），均可重新开局。
